...
**Drum Machine for Scale Practice — done, and still current.** Scale Practice's single quarter-note click was replaced with a synthesized multi-voice drum machine — kick/snare/closed-hat/open-hat, a 16-step pattern with swing, and 5 curated genre presets — scheduled by a sample-accurate Web Audio lookahead scheduler, with an optional chord-progression backing (curated templates only — the custom-progression builder was part of the Saved Material Library that was later removed) and per-chord scale highlighting on the fretboard. See `AGENTS.md` §26/§27 for the full current mechanics and doctrine.

...
**Radio Mode: an autonomous 24/7 stream to promote FretField on YouTube (user-requested, 2026-08).** The user's idea: "develop a feature that allows fretfield to play electronic music autonomously, so that I can stream it live all day." Discussed and scoped up front (three explicit product decisions, not assumptions): a standalone unlisted page rather than a third destination or a Practice mode (keeps AGENTS.md §1's "deliberately two destinations" rule untouched), constrained randomization rather than a hand-authored playlist, and a new audio-reactive visualizer rather than reusing the fretboard. Actually broadcasting to YouTube is out of scope -- that's OBS pointing a Browser Source at the page and encoding the stream; FretField's job is just to run forever and look/sound good.

Built entirely on top of the existing Groove Engine + generated Acid Bass -- reuses the `scalePractice` singleton and its own existing public setters as the whole playback engine, no parallel scheduler, no new DSP. `RadioDirector` (`$lib/stores/radio-director.ts`) is the one new piece of playback logic: a small store-independent module (five injected setter functions) that rolls a fresh root/progression/groove-preset/Acid-Bass-style/tempo combo every 90-180 seconds, guarded so no field ever repeats back-to-back and tempo is picked from a genre-appropriate range per groove preset (a flat global BPM range would put bossa nova and drum-and-bass in the same pool). Closed a real correctness risk before anything else: Radio drives the same singleton store for hours, so a new `persistEnabled` flag (default `true`, zero change for ordinary Practice) suppresses `persist()` entirely, so Radio's own constant rotation can never silently overwrite a real user's saved Practice session in the same browser's `localStorage`. A new `masterAnalyser` on the store (fed by `drumsBus.output`/a new public `ChordPadFxBus.output` field/`acidBassVoice`'s existing `outputAnalyser`, alongside their existing connections to `ctx.destination`) feeds `RadioVisualizer.svelte`, a full-bleed audio-reactive radial spectrum ring in the app's own yellow/black brand palette plus a "now playing" text overlay -- the same "genuine live tap, not a modeled re-creation" idiom `AcidBassAudioScope.svelte` already established, just full-screen and circular. `src/routes/radio/+page.svelte` shows a one-time "Click to start" overlay (Web Audio's own user-gesture requirement, the same one every other Play button in the app already goes through), is excluded from search (`noindex, nofollow`, not in `static/sitemap.xml`), and is never linked from anywhere in the app.

Verified live via a real accelerated soak: 120 forced rotations in quick succession (via a `forceRotate()` test hook mirroring the existing `installLiveInputTestHooks`/`installPersistenceTestHooks` seams) produced no console errors and left the page rendering correctly -- `setAcidBassGenerationStyle` (the one Director-driven setter that touches Acid Bass) only ever writes a plain data field feeding the pure bassline generator, never the live `AcidBassVoice`/its patch, so there was no live Web Audio node in that path to leak in the first place.

**Fixed: Radio Mode's bass was never audible (user-reported, 2026-08 -- "i can't hear the bass playing").** Root cause: every rotation calls `setGroove(groovePreset.groove)`, and a `GroovePreset`'s own baked-in `Groove` carries Acid Bass's default disabled/manual state (`createEmptyGroove`'s own default) -- so `setGroove` alone silently turned the bass back off on every single rotation, immediately undoing the enabled/generated state set at Radio's own startup, before the transport ever ticked once. Fixed by adding `setAcidBassEnabled`/`setAcidBassMode` to `RadioDirectorDeps` and re-asserting both (`true`/`'generated'`) right after `setGroove`, every rotation -- verified live via the new `getAcidBassState()` test hook (10 rotations in a row, `{ enabled: true, mode: 'generated' }` every time) and locked in with a unit regression test asserting the call ordering. Same request also pinned the groove/genre to Trance only ("music should always be trance") -- `GROOVE_PRESET_IDS` narrowed from the full preset pool to `['trance']`; root/progression/Acid-Bass-style still rotate normally, only the genre itself no longer varies.

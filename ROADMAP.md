...
Phase 6 (Chord builder — composable extensions/alterations beyond the 11 base chord qualities) and Phase 7 (shareable state polish beyond the URL state already shipped) remain open, along with everything from Phase 10 onward (remaining practice modes — Ear Training, Groove Navigation, Walking Bass — audio playback beyond Scale Practice's drum machine, alternate tunings, educational integration).
...
**Saved Material Library — done.** A local, account-free "personal harmonic notebook" (external roadmap Phase 18): named Scale Maps (save/load/rename/delete a Custom Scale Map's blocks), Favorite Chords (a one-tap root+chord bookmark row in Chord Explorer), user-defined Progressions (a degree-by-degree builder — interval-from-tonic plus chord quality — alongside the 5 curated templates, resolved via the same engine so a saved progression behaves identically to a curated one), and My Presets (save the current Guided Practice or Scale Practice setup, browsable alongside the 15 curated presets in Practice → Presets). All four share one `createSavedCollectionStore` factory and are local-only, matching every persistence decision this session — no shareable-URL export/import yet, that's the one piece of Phase 18's fuller ambition still deferred. See `/Users/alessandro/.claude/plans/ancient-soaring-squid.md` for the full milestone-by-milestone build log.

**Drum Machine for Scale Practice — done.** Scale Practice's single quarter-note click (`src/lib/audio/metronome.ts`, now deleted) was replaced outright with a synthesized multi-voice drum machine — kick/snare/closed-hat/open-hat, a 16-step pattern with swing, and 5 curated genre presets (Click, Straight/Rock, Blues Shuffle, Jazz Swing, Funk) — scheduled by a sample-accurate Web Audio lookahead scheduler. Custom patterns are saveable via the Saved Material Library's `createSavedCollectionStore` alongside Scale Maps/Progressions/Presets/Favorites. This explicitly supersedes `AGENTS.md`'s former "quarter-note clicks only, no subdivisions/swing/accents" doctrine for Scale Practice, by direct product request — every other constraint in that doctrine (no automatic tempo ramps, no mute toggle, no persistent session stats, no per-beat grading, no Live Input/Guided Practice driving this mode) still holds. Stays entirely inside Scale Practice — not the separately-frozen "Groove Navigation"/"Backing Tracks" future modes. See `/Users/alessandro/.claude/plans/ancient-soaring-squid.md` for the full milestone-by-milestone build log.

Everything beyond the roadmap's own "1.0 boundary" (a harmonic-fluency model, daily practice planning, donations, alternate tunings, shareable-URL saved-material export/import) remains deliberately out of scope, to be picked up in a later planning effort.

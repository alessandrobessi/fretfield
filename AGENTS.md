...
Pure TypeScript, the chord-backing pad's FX rack (Reverb/Delay/Chorus, then Phaser/Flanger/Tremolo -- both user-requested, 2026-08, built in two stages). Deliberately a separate domain from `$lib/acid-bass/`, not nested inside it, even though both are Groove Engine accompaniment layers -- the chord pad has no arrangement/pattern/step concept of its own (it just sustains whichever chord the progression names, see `chord-voices.ts`), so this is FX-only state.

```text
types.ts resolve.ts pattern.ts migrate.ts
```

`types.ts` holds `ChordPadFxState` (`version: 2`, plus `reverb`/`delay`/`chorus`/`phaser`/`flanger`/`tremolo` sub-patches, each `enabled` + a small number of macros -- `tremolo` alone has no `mix` field, since it's a genuine in-series amplitude modulation rather than a dry/wet insert like the other five, see its own doc comment) -- `ChordPadDelayPatch` deliberately mirrors `AcidDelayPatch`'s shape (same division set, same feedback/mix semantics) but is its own type, not a shared import, since `acid-bass/delay.ts` documents itself as scoped to Acid Bass only. `resolve.ts` is the pure macro-to-DSP math the actual node graph (`$lib/audio/chord-pad-fx.ts`) reads from -- notably `reverbSizeToFeedbackGain`/`reverbDampingToLowpassHz`, tuned for a small algorithmic comb+allpass reverb rather than a convolution one (see that file's own header for why). `pattern.ts`'s `createDefaultChordPadFxState()` has every effect off, matching every other new-modulation precedent in this app -- an existing or migrated groove must sound identical until a player turns something on. `migrate.ts`'s `coerceChordPadFxState(raw)` runs a real version-branch migration: a persisted `version: 1` groove (Reverb/Delay/Chorus only, the shape before Phaser/Flanger/Tremolo existed) keeps those three fields exactly as saved and defaults the three new ones off/neutral, mirroring `acid-bass/migrate.ts`'s own version-branch precedent -- every field is still defensively coerced regardless of claimed version.

...
Pure TypeScript, the Acid Bass Engine's domain model — no Svelte, no `AudioContext`. A monophonic, 303-inspired synth bass voice living inside Scale Practice's Groove Engine as a third accompaniment layer alongside drums and the chord pad, sharing the drum engine's persistence, migration, and `A`/`B`/`F`/`T` arrangement roles — there is no independent bass arrangement (see §26 for the full product scope and "must not add" list).

```text
types.ts pattern.ts resolve.ts
```

`types.ts` is the data model: `AcidBassStep` (`active`/`interval: IntervalId`/`octave: AcidOctaveOffset`/`accent`/`slide`), `AcidBassPattern` (an array of steps, one per role, resized in lockstep with the drum patterns), `AcidBassPatch` (`wave`/`tone`/`resonance`/`motion`/`decay`/`drive`, all `0`–`100` macro controls — never raw Hz/Q/etc in the stored model, only in `resolve.ts`'s mapping functions), and `AcidBassState` (`enabled`, `patch`, `patterns: Record<PatternRole, AcidBassPattern>`). Deliberately reuses the app's existing `IntervalId` (`$lib/music/intervals.ts`) for `AcidBassStep.interval` rather than inventing a second numeric interval type — one canonical interval representation, one label convention, everywhere in the app. `pattern.ts` holds the pure mutators (`setAcidStepActive`/`Interval`/`Octave`, `toggleAcidStepAccent`/`Slide`, `resizeAcidPattern`, `resizeAcidBassState`, `createDefaultAcidBassState`) — same shape as `groove/pattern.ts`'s drum-pattern mutators, deliberately, so both feel like the same engine rather than two. `resolve.ts` is pure mapping/resolution: `resolveAcidStepMidi` (interval + octave + a per-bar harmonic root → a MIDI number, reusing `$lib/music/intervals`'s `intervalSemitones` — never a second harmony engine) and the patch-macro-to-DSP-parameter mappings (`toneToCutoffHz`, `resonanceToQ`, `motionToEnvelopeRatio`, `decayToSeconds`, `driveToPregain`) that `acid-bass-voice.ts` (`$lib/audio/`, see above) consumes. Nothing here schedules anything or knows about `AudioContext.currentTime` — that stays in `scale-practice.svelte.ts`/`GrooveTransport`, same boundary the rest of the Groove Engine keeps.
...
That same Groove Engine also carries an optional Acid Bass Engine (`$lib/acid-bass/` — types/pattern/resolve, see §4 — driving `$lib/audio/acid-bass-voice.ts`, all called from `scale-practice.svelte.ts`): a monophonic, 303-inspired synth bass voice, off by default, toggled via "Bass On/Off" in `PracticeSessionBar.svelte`, with its own patch macros (Wave — Saw/Square, Tone, Resonance, Motion, Decay, Drive, all `0`–`100`) on `BandPanel.svelte`'s Bass tab and its own step grid + selected-step editor (Active, Interval, Octave, Accent, Slide) inside `GrooveEditor.svelte`'s "Bass Steps" sub-tab (a "Drum Steps | Bass Steps" toggle switches which pattern the grid edits — both read the _same_ `selectedPatternRole`, since Acid Bass shares the drum engine's `A`/`B`/`F`/`T` arrangement rather than an independent one). Its interval storage reuses the app's existing `IntervalId` (`$lib/music/intervals.ts`), not a bespoke numeric type — one canonical interval representation everywhere, per §3.1. Within Acid Bass specifically, do not add: DAW features (no piano-roll, no automation lanes, no MIDI export), a hardware-clone 303 UI (no fake knobs/faceplate skeuomorphism — controls stay this app's existing slider/select language), AI-generated bass lines (already covered app-wide above, restated because it's the single most likely opportunistic addition here), a second `AudioContext` or a second scheduler (schedules off the exact same `GrooveTransport`/swung `stepOffsetMs` timing the drums already use — see `scale-practice.svelte.ts` above), an independent bass arrangement or pattern-role set (always the drum engine's own `A`/`B`/`F`/`T`, via the same `selectedPatternRole`), cross-bar slide (a step's `slide` only glides to the _immediately following_ step, never across a bar boundary — deferred, not a v1 requirement), an Intensity/velocity-gating concept of its own (Acid Bass has no `minIntensity` equivalent — every active step always sounds), sample-based tone generation (stays two oscillators + a filter, no asset-loading pipeline), or persistence/config separate from the rest of the Groove Engine (it lives inside `Groove.acidBass`, migrates and saves/loads with the rest of a groove, including "My Grooves" — never a parallel storage key).
...
audio-input.ts fake-audio-source.ts drum-voices.ts chord-voices.ts acid-bass-voice.ts
```

...
`drum-voices.ts`/`chord-voices.ts`/`acid-bass-voice.ts` are this directory's one exception to "acoustic-pitch domain only": they're audio _output_ (the Groove Engine's synthesized drum kit, its optional chord-progression backing pad, and the optional Acid Bass synth voice — see `src/lib/groove/`/`src/lib/acid-bass/` below), not analysis. They still must not import anything theory-related — no scales, no chord/key concepts, only step timing, synthesized percussion/oscillators, and plain Hz frequencies/MIDI numbers and durations. `chord-voices.ts`'s `triggerChordPad(ctx, destinationNode, time, frequenciesHz, durationSeconds, gain?)` takes frequencies, never a chord/root/interval — `scale-practice.svelte.ts` resolves theory (via `$lib/music/chords`'s `required` intervals and `$lib/music/intervals`'s `intervalSemitones`) before calling in, same boundary `drum-voices.ts` already keeps. `destinationNode` (user-requested, 2026-08) is where each hit's final envelope connects instead of `ctx.destination` directly — in practice always `chord-pad-fx.ts`'s `createChordPadFxBus(ctx).input`, so every chord-pad hit passes through the pad's own seven-effect rack (Reverb/Delay/Chorus, then Phaser/Flanger/Tremolo, then Fuzz, built in three stages) before reaching the real output. That bus is a separate self-contained domain, `$lib/chord-pad-fx/` (types/resolve/pattern/migrate, mirroring `$lib/acid-bass/`'s own split — see its file listing below), nested on `Groove` as `chordPadFx: ChordPadFxState`, sibling to `acidBass` and independently migrated/coerced. Reverb is a small algorithmic (Freeverb/Schroeder-style) comb+allpass network built entirely from `DelayNode`/`GainNode`/`BiquadFilterNode` (see `chord-pad-fx.ts`'s own file header) — not a convolution reverb, since this app has no external impulse-response asset anywhere. `drum-voices.ts`'s six trigger functions (`triggerKick`/`triggerSnare`/`triggerClosedHat`/`triggerOpenHat`/`triggerRide`/`triggerRim`) all take a plain `gain` parameter — that's how step velocity (ghost/normal/accent) reaches the actual sound, with no theory or sequencing concept inside this file. `acid-bass-voice.ts`'s `createAcidBassVoice(ctx, destination?)` is architecturally different from the other two by necessity, not inconsistency: `drum-voices.ts`/`chord-voices.ts` build a fresh node graph per hit/chord (stateless, fire-and-forget trigger functions), but Acid Bass's slide/legato behavior needs one long-lived monophonic node graph that persists across the whole playback session — `AcidBassVoice`'s `setPatch`/`schedule`/`setTempo`/`silence`/`dispose` interface, not a plain `trigger*` function. As of the V2 engine (`~/Downloads/ACID-BASS-ENGINE-V2.md`) plus a later Osc 2/dual-LFO follow-up, that graph is substantially larger than "two oscillators and a filter": four always-running main-wave oscillators (saw/square/triangle/Pulse) crossfaded by waveform, an optional sub oscillator, a second full oscillator (Osc 2 — a single type-switched node, the same simplification Sub uses, own independent tune/fine/level), a pre-filter saturation stage, a filter section that runs the Biquad path (`legacy`/`svf12`) and the `acid24` `AudioWorkletNode` (`acid-worklet-node.ts`) permanently in parallel and crossfades between them, a Pulse oscillator that does the same crossfade trick between a static `PeriodicWave` and its own live-PWM `AudioWorkletNode`, and two independent free-running LFOs (`acid-bass-lfo.ts`, one instance each) each routed to whichever single destination its own patch slot names via its own bank of depth-scaling gains — still not a many-to-many modulation matrix. Every one of those "two implementations of the same thing" pairs uses the identical idiom: both stay connected all the time, only their crossfade gain changes, so switching between them (or a worklet finishing its async load) never audibly glitches — worth knowing before adding a fourth. `sequencer.ts` (probability/ratchet/parameter locks) and the migration/factory-patch modules stay outside this file entirely — see the `$lib/acid-bass/` entry below for the full module breakdown. `scale-practice.svelte.ts` still resolves all theory (interval → MIDI, via `$lib/acid-bass/resolve.ts`'s `resolveAcidStepMidi` and `$lib/audio/note-mapping.ts`'s existing `midiToFrequency`) before calling in — `acid-bass-voice.ts` itself only ever sees plain Hz/MIDI, same boundary. All three share `scale-practice.svelte.ts`'s one `AudioContext`, entirely separate from the capture context `audio-input.ts` owns — never merged into one context, and never a second `AudioContext` for Acid Bass specifically. **The Mixer** (user-requested, 2026-08): all three channels now share one `volumeToGain` headroom curve, moved out of `acid-bass/resolve.ts` into `audio/gain.ts` since it's no longer Acid-Bass-specific. `drum-voices.ts`'s six trigger functions each gained the same `destinationNode` parameter `triggerChordPad` already had, and `drum-voices.ts` also exports `createDrumsBus(ctx)` — a persistent `GainNode` + `setVolume(value, atTime?)`, the same "one bus, hide the raw AudioParam" shape `chord-pad-fx.ts`'s `ChordPadFxBus` already used, extended here with its own `setVolume`/`channelGain` stage appended after the FX rack's existing dry/wet sum. `scale-practice.svelte.ts` owns one `DrumsBus` instance (created/nulled alongside `chordPadFxBus`/`acidBassVoice` in `start()`/`stop()`) and two new top-level `$state` fields, `drumsVolume`/`chordsVolume` (0-100, default 100, persisted like `intensity` — session state, not `Groove` content). Bass has no third field: its fader is `patch.output.volume`, already persisted inside `Groove.acidBass` and already exposed as the OUTPUT panel's own Volume knob — the Mixer's Bass fader and that knob are the same control, not two synchronized ones. **Radio Mode** (user-requested, 2026-08) added a `masterAnalyser` to `scale-practice.svelte.ts`'s `start()`, fanning `drumsBus.output`/`chordPadFxBus.output`/`acidBassVoice.outputAnalyser` into one new tap (exposed via `getMasterAnalyser()`) alongside their existing connections to `ctx.destination` — `ChordPadFxBus` gained a matching public `output` field for this, mirroring `DrumsBus`'s own; nothing about the existing per-voice routing changed. See AGENTS.md's own Radio Mode paragraph above and BLUEPRINT.md §21 for the full feature.
...
Pure TypeScript, the chord-backing pad's FX rack (Reverb/Delay/Chorus, then Phaser/Flanger/Tremolo, then Fuzz -- all user-requested, 2026-08, built in three stages). Deliberately a separate domain from `$lib/acid-bass/`, not nested inside it, even though both are Groove Engine accompaniment layers -- the chord pad has no arrangement/pattern/step concept of its own (it just sustains whichever chord the progression names, see `chord-voices.ts`), so this is FX-only state.

```text
types.ts resolve.ts pattern.ts migrate.ts
```

`types.ts` holds `ChordPadFxState` (`version: 3`, plus `fuzz`/`reverb`/`delay`/`chorus`/`phaser`/`flanger`/`tremolo` sub-patches, each `enabled` + a small number of macros -- `tremolo` alone has no `mix` field, since it's a genuine in-series amplitude modulation rather than a dry/wet insert like the other six, see its own doc comment) -- `ChordPadDelayPatch` deliberately mirrors `AcidDelayPatch`'s shape (same division set, same feedback/mix semantics) but is its own type, not a shared import, since `acid-bass/delay.ts` documents itself as scoped to Acid Bass only; `ChordPadFuzzPatch`'s own `drive` mirrors Acid Bass's `driveToPregain` idiom -- a pre-gain into one fixed clipping curve, not a per-value regenerated one. `resolve.ts` is the pure macro-to-DSP math the actual node graph (`$lib/audio/chord-pad-fx.ts`) reads from -- notably `reverbSizeToFeedbackGain`/`reverbDampingToLowpassHz`, tuned for a small algorithmic comb+allpass reverb rather than a convolution one (see that file's own header for why). `pattern.ts`'s `createDefaultChordPadFxState()` has every effect off, matching every other new-modulation precedent in this app -- an existing or migrated groove must sound identical until a player turns something on. `migrate.ts`'s `coerceChordPadFxState(raw)` runs a real version-branch migration at each step: a persisted `version: 1` groove (Reverb/Delay/Chorus only) keeps those three fields exactly as saved and defaults `phaser`/`flanger`/`tremolo`/`fuzz` off/neutral; a `version: 2` groove (adds Phaser/Flanger/Tremolo) keeps all six of those and defaults only `fuzz` off/neutral -- mirroring `acid-bass/migrate.ts`'s own version-branch precedent -- every field is still defensively coerced regardless of claimed version.
...
`src/lib/components/hardware/` holds the 2026 visual-rebrand's reusable primitives: `Led.svelte` (off/active/current, with a static glow plus an optional `prefers-reduced-motion`-safe pulse — every animated state has a non-motion equivalent, per §17), `HardwarePanel.svelte` (the branded yellow-chassis/carbon-panel wrapper; `tone="yellow"` is reserved for the handful of modules the rebrand calls out — Groove Engine, Acid Bass — not general app chrome), `HardwareButton.svelte` (primary yellow/black and secondary black/yellow variants, with a shared `pressed` deep-yellow state for toggle buttons), and `Knob.svelte` (a real rotary control: vertical-drag plus full keyboard support — arrows/Home/End/PageUp/PageDown — `role="slider"`, `aria-valuenow` always exposed but no visible numeric readout, by explicit user preference; optional `disabled`/`title` props, added 2026-08 for the Groove Engine's Amount knob — `disabled` dims it, sets `tabindex="-1"`/`aria-disabled`, and blocks pointer/keyboard input, `title` carries an explanatory tooltip through to the same element `aria-label` lives on, e.g. "12/8 already has its own compound feel"; used for Acid Bass's patch macros — three dozen-plus across its VCO/SUB/OSC 2/VCF/ENV/LFO 1/LFO 2/OUTPUT panels — and, as of the same 2026-08 pass, the Groove Engine's own Amount/Intensity, which used to be the one pair of plain `<input type="range">` sliders left in the app until direct user feedback ("can the sliders be more aligned to the style of the app?") asked for them to match; there is no longer a "Knob vs. plain range" split anywhere in Scale Practice's UI). Toggle/switch state elsewhere in the app still uses the existing `aria-pressed`-button pattern directly rather than a dedicated Switch component — a deliberate scope decision, not an oversight.
...
- user profiles
- guitar mode
- LLM/API/model-generated bass lines (local deterministic algorithmic bassline generation is separately authorized by Acid Bass Intelligence V4 — see the dedicated note after the Acid Bass paragraph below)
- notation editor
- tablature editor
- DAW features
- analytics SDKs
...
That same transport can optionally layer an audible chord-progression backing underneath the beat (`$lib/audio/chord-voices.ts`'s synthesized pad, off by default, picked via `ProgressionSelector` in `PracticeSessionBar.svelte` — see §4). Within that addition specifically, do not add: an independent tonic for the progression (it always derives from `scalePractice.root`), per-chord duration overrides (one `barsPerChord` applies to every chord), voice-leading between consecutive chords (every chord is the same simple closed voicing, root position, tones ascending), or a way to build/save a custom progression (that feature was removed — only the curated template list remains). The pad does now have its own seven-effect FX rack (Fuzz/Reverb/Delay/Chorus/Phaser/Flanger/Tremolo, `$lib/chord-pad-fx/`/`$lib/audio/chord-pad-fx.ts` — user-requested, 2026-08, Harmony tab, built in three stages), controlled via its own `HardwarePanel`s, off by default per effect. **Independent drum/chord/bass volume mixing was later explicitly authorized and built (the Mixer, user-requested, 2026-08) — superseding this paragraph's own former "do not add" listing of it; see the Mixer's own entry below (§4, `drum-voices.ts`/`chord-pad-fx.ts`/`audio/gain.ts`) and BLUEPRINT.md §21.**

That same Groove Engine also carries an optional Acid Bass Engine (`$lib/acid-bass/` — types/pattern/resolve/migrate/sequencer/factory-patches/transforms/acid24-ladder/pulse-oscillator, see §4 — driving `$lib/audio/acid-bass-voice.ts`/`acid-bass-lfo.ts`/`acid-worklet-node.ts`, all called from `scale-practice.svelte.ts`): a monophonic, 303-inspired synth bass voice, off by default, toggled via "Bass On/Off" in `PracticeSessionBar.svelte`. Following a separately-supplied V2 spec (`~/Downloads/ACID-BASS-ENGINE-V2.md`) and a later Osc 2/dual-LFO follow-up, its once-flat six-macro patch grew into a nested `oscillator`/`filter`/`envelope`/`glide`/`lfo1`/`lfo2`/`modulation`/`distortion`/`delay`/`output` shape (the last three added by V4 M15–M17, see below) exposed across `BandPanel.svelte`'s Bass tab as twelve nested VCO/SUB/OSC 2/VCF/ENV/LFO 1/LFO 2/ENV MOD/ACCENT MOD/RANDOM MOD/DELAY/OUTPUT panels laid out as a compact responsive grid, every control always visible (no disclosure — see §4's `AcidBassControls.svelte`-adjacent hardware-component entry), a toggleable "GLOSSARY" reference panel (`glossary.ts`, plain data, one line per control) next to the Patch picker, plus a 14-preset factory-patch picker (`factory-patches.ts`: 8 original V2 sound-design presets + 6 genre-flavored ones added at V4's M22 — House Deep/RnB Velvet/DnB Reese/Techno Drive/Trance Pluck/Bossa Nova) and pattern-wide transform buttons (Rotate L/R, Simplify, Densify, Octave shift, Clear All Locks — `transforms.ts`, deliberately basic/not chord-or-scale-aware) inside `GrooveEditor.svelte`'s "Bass Steps" sub-tab, alongside its step grid + selected-step editor (now also Probability/Ratchet/Gate/a parameter-lock disclosure, not just Active/Interval/Octave/Accent/Slide) — a "Drum Steps | Bass Steps" toggle switches which pattern the grid edits, both reading the _same_ `selectedPatternRole`, since Acid Bass shares the drum engine's `A`/`B`/`F`/`T` arrangement rather than an independent one. Its interval storage reuses the app's existing `IntervalId` (`$lib/music/intervals.ts`), not a bespoke numeric type — one canonical interval representation everywhere, per §3.1. A step's `slide` glides to the immediately following step, or — if `crossBarSlide` is on (default for new patches, off for anything migrated from V1) — the next bar's first active step; `ratchet > 1` silently drops that step's own outgoing slide (undefined which of several hits would glide) and disables the Slide checkbox in the UI. The `acid24` filter model and the Pulse oscillator's live PWM both run on dedicated `AudioWorkletNode`s (`static/acid-filter-processor.js`/`static/acid-pulse-oscillator-processor.js`) that silently fall back to a Biquad/`PeriodicWave` approximation if the worklet never loads — never a user-facing error. Within Acid Bass specifically, do not add: DAW features (no piano-roll, no automation lanes, no MIDI export), a hardware-clone 303 UI (no fake knobs/faceplate skeuomorphism — controls stay this app's existing slider/select language, now including real rotary `Knob`s), LLM/API/model-generated bass lines (already covered app-wide above; V4's own _local, deterministic, algorithmic_ generator is a distinct, explicitly authorized thing — see the note immediately below), a second `AudioContext` or a second scheduler (schedules off the exact same `GrooveTransport`/swung `stepOffsetMs` timing the drums already use — see `scale-practice.svelte.ts` above), an independent bass arrangement or pattern-role set (always the drum engine's own `A`/`B`/`F`/`T`, via the same `selectedPatternRole`, even in V4's generated mode), an independent bass tempo/swing/meter, polyphony, a many-to-many modulation matrix (V4 authorizes exactly five fixed single-destination sources, not a matrix — see below), a general effects rack (V4 authorizes exactly one dedicated tempo-sync delay, not an FX rack — see below), chord/scale-aware pattern transforms on the existing _manual_ patterns (the transforms above are deliberately basic — see `transforms.ts`'s own doc comment; this doesn't apply to V4's separate generated-line pipeline, which is chord/scale-aware by design), an Intensity/velocity-gating concept of its own (Acid Bass has no `minIntensity` equivalent — every active step always sounds, modulo its own `probability` roll), sample-based tone generation (stays synthesized oscillators + filters, no asset-loading pipeline), or persistence/config separate from the rest of the Groove Engine (it lives inside `Groove.acidBass`, migrates and saves/loads with the rest of a groove, including "My Grooves" — never a parallel storage key).

**The Mixer** (user-requested, 2026-08 — "I think we need a mixer to handle the volume of drums, pads, and bass"): a Drums/Chords/Bass channel-volume strip on its own `BandPanel.svelte` tab, between Bass and Editor. **Originally built as an always-visible strip above the Drums/Harmony/Bass/Editor tabs — corrected by direct user feedback** ("i've just realized that it is everywhere ... mixer should have its own tab, it should not be in the harmony section") **to a normal tab instead, same shape as the other four; do not move it back to an always-visible strip.** Each fader is a rotary `Knob` (`$lib/components/hardware/Knob.svelte`), the same control every other volume/level knob in the app uses — not a plain `<input type="range">`, which is how it originally shipped until direct user feedback ("can the sliders be more aligned to the style of the app?") asked for it to match; don't reintroduce a raw range input here. See §4's audio-output entry for the routing (`audio/gain.ts`'s shared `volumeToGain`, `drum-voices.ts`'s `createDrumsBus`, `chord-pad-fx.ts`'s `channelGain`/`setVolume`) and BLUEPRINT.md §21 for the full writeup. Do not add: a fourth Master fader (out of scope — the user asked for exactly drums/pads/bass), per-effect sends within the Mixer tab itself (that's what each voice's own FX/OUTPUT panel is for), or a second, disconnected volume field for Bass (the Mixer's Bass fader must stay the same control as the OUTPUT panel's existing Volume knob, never a shadow copy that can drift out of sync).

**Acid Bass Intelligence V4 authorization** (`~/Downloads/ACID-BASS-INTELLIGENCE-V4.md`, in progress as of this note — implemented incrementally across an explicit milestone sequence, not all landed yet; check `ROADMAP.md` for current status before assuming a piece described here already exists): Acid Bass may consume harmonic intelligence from new pure `$lib/music/bassline/` modules (a bassline generator: rhythm → harmonic candidates → chord/key/voice-leading selection → controlled chromaticism → register/playability realization → articulation → explanation). Generated basslines are explicitly authorized — deterministic, local, algorithmic, and kept on the theory side of the existing theory/audio boundary (§3.1/§3.2), never inside `acid-bass-voice.ts`. This authorization does **not** restore the removed Progression Field or Voice-Leading Paths product modes (§1) — V4's `voice-leading.ts` is new, internal-only bass-generator scoring logic, not a UI feature, and must not import or resurrect the deleted `connection-score.ts`/`voice-leading-paths.ts`/`local-fields.ts`. It narrows three other doctrine points, each _only_ to the exact extent V4 specifies, nothing more: (1) the modulation-matrix prohibition now permits exactly five fixed single-destination sources (LFO 1, LFO 2, ENV, Accent, Random) — still never many-to-many; (2) the effects-rack prohibition now permits exactly one dedicated tempo-synced delay — still no reverb/chorus/EQ/compressor/general FX rack; (3) Scale Practice's "no grading" doctrine (§4's `positions.ts` entry, §26's Groove Engine paragraph above) is narrowed _only_ for a dedicated, opt-in Acid Bass training session ("Learn This Line," pitch-only, no timing/groove grading) — ordinary Scale Practice itself continues to have zero grading/evaluation concept. Manual Acid Bass (`mode: 'manual'`) is unchanged and remains the default/migration mode; generated mode (`mode: 'generated'`) is strictly additive.

**Radio Mode** (`src/routes/radio/+page.svelte`, user-requested, 2026-08 — an autonomous 24/7 stream to promote FretField on YouTube): **not a third destination and not a Practice mode toggle — a standalone, unlisted page, reached only by direct URL, never linked from Explore/Practice's own nav.** This is a deliberate product decision (discussed and confirmed with the user before building), not an oversight: it keeps §1's "deliberately two destinations" rule completely untouched. Do not add a nav link to it, do not add it to `static/sitemap.xml`, and do not remove its `noindex, nofollow` meta tag — all three would make it a discoverable product surface, which it is explicitly not. It reuses the `scalePractice` singleton and its own existing public setters as its entire playback engine — no parallel scheduler, no second `AudioContext`, no new DSP. The only genuinely new logic is `RadioDirector` (`$lib/stores/radio-director.ts`, see §4), which just calls those setters on a timer. Three small, additive-only changes exist on the shared store/bus code specifically to support this: `persistEnabled` (a flag guarding `persist()`, since Radio drives this singleton for hours and must never silently overwrite a real user's own saved Practice session in the same browser's `localStorage`), `masterAnalyser`/`getMasterAnalyser()` (fans `drumsBus.output`/`chordPadFxBus.output`/`acidBassVoice.outputAnalyser` into one new tap, alongside their existing connections to `ctx.destination` — nothing rerouted), and `ChordPadFxBus`'s new public `output` field (mirroring `DrumsBus`'s own, needed for the analyser tap). Do not add Radio-specific UI, settings, or persistence to ordinary Practice — everything Radio-specific lives under `src/routes/radio/` and `$lib/components/radio/`. **The groove/genre is pinned to Trance only** (user-requested, 2026-08 — "music should always be trance") — `radio-director.ts`'s `GROOVE_PRESET_IDS` is `['trance']`, not the full preset pool; root/progression/Acid-Bass-style still rotate normally. **`RadioDirector` must call `setAcidBassEnabled(true)`/`setAcidBassMode('generated')` on every single rotation, not just once at start** — this is a fixed regression, not defensive padding: a `GroovePreset`'s own baked-in `Groove` carries Acid Bass's default disabled/manual state, so `setGroove` alone silently turns the bass back off every time it's called; a user reported this exact symptom ("i can't hear the bass playing") before the fix. Do not remove those two calls from `applyCombo`, and if `RadioDirectorDeps` ever grows another `setGroove`-adjacent setter that resets sibling `Groove` state, re-assert whatever it clobbers the same way.
...
chord-pad-fx.ts acid-bass-voice.ts acid-bass-lfo.ts acid-worklet-node.ts
gain.ts
```
...
And in `src/lib/chord-pad-fx/` (the chord-backing pad's FX rack — see §4):

...
**The Mixer** (user-requested, 2026-08 — "I think we need a mixer to handle the volume of drums, pads, and bass"): a Drums/Chords/Bass channel-volume strip on its own `BandPanel.svelte` tab, between Bass and Editor. **Originally built as an always-visible strip above the Drums/Harmony/Bass/Editor tabs — corrected by direct user feedback** ("i've just realized that it is everywhere ... mixer should have its own tab, it should not be in the harmony section") **to a normal tab instead, same shape as the other four; do not move it back to an always-visible strip.** See §4's audio-output entry for the routing (`audio/gain.ts`'s shared `volumeToGain`, `drum-voices.ts`'s `createDrumsBus`, `chord-pad-fx.ts`'s `channelGain`/`setVolume`) and BLUEPRINT.md §21 for the full writeup. Do not add: a fourth Master fader (out of scope — the user asked for exactly drums/pads/bass), per-effect sends within the Mixer tab itself (that's what each voice's own FX/OUTPUT panel is for), or a second, disconnected volume field for Bass (the Mixer's Bass fader must stay the same control as the OUTPUT panel's existing Volume knob, never a shadow copy that can drift out of sync).

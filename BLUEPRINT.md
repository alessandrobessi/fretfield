...
- **Acid Bass Intelligence V4** (`~/Downloads/ACID-BASS-INTELLIGENCE-V4.md`, in progress as of this note — implemented incrementally across an explicit milestone sequence; check `ROADMAP.md` for current status): turns Acid Bass from a synth attached to a manual step sequencer into an algorithmic bassline generator, layered strictly on top of the architecture above. A new pure `$lib/music/bassline/` domain (rhythm → harmonic candidates → chord/key/voice-leading selection → controlled chromaticism → register/playability realization → articulation → explanation) produces a `GeneratedBasslinePlan`, derived on demand and never persisted — only the generation settings (style, harmony mode, seed, density, chromaticism, movement, register, playability, intelligence) and the seed are saved, alongside the unchanged manual `A`/`B`/`F`/`T` patterns. A new `AcidBassMode` (`'manual' | 'generated'`) selects between them; manual stays the default/migration mode, unchanged from V3. Generation uses the existing progression/per-chord-scale/zone/tuning context already in Scale Practice — never invents a hidden chord, and stays disabled with an explanatory message when no root/progression is chosen. Sound expression grows in parallel but stays bounded exactly as this doctrine specifies: five fixed single-destination modulation sources (LFO 1, LFO 2, ENV, Accent, Random — never a matrix), three distortion characters (Soft/Diode/Hard, Soft byte-compatible with the existing V3 curve), and one dedicated tempo-synced delay (never a general FX rack). Two opt-in training additions — "Learn This Line" (pitch-only evaluation against a generated bar, via the existing Live Input note-onset stream) and "Shadow Mode" (the fretboard previews upcoming generated notes during ordinary playback) — are the only exception carved into Scale Practice's otherwise absolute "no grading" doctrine, and only because they're a dedicated opt-in Acid Bass session, not ordinary Scale Practice. Still one `GrooveTransport`, one output `AudioContext`, and the same `A`/`B`/`F`/`T` arrangement — generated mode is not a second bass arrangement or a second scheduler.

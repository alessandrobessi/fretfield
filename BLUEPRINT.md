...
Scale Practice          "Can you play this scale in time?" — every note of a chosen scale/key/fret zone is highlighted at once, whatever's played is highlighted live, and a free-standing metronome keeps time alongside it
```
...
time, whether or not it's in the scale
        │
        ▼
Start/Stop only starts/stops the drum machine (a synthesized multi-voice
groove at the chosen BPM/pattern/swing) — it has no effect on what's
highlighted either way
```

**Session state** (`ScalePracticeStore`): `root`, `scaleId`, `zone: { minFret, maxFret }`, `bpm` (30–240, default 80), `pattern: GroovePattern` (a 16-step boolean grid per voice plus a swing amount, see below), `running` (the drum machine only). Configured independently of every other mode — switching to another tab stops the drum machine (component-unmount cleanup), but root/scale/zone/tempo/pattern choices persist for next time (unlike Scale Blocks' `chordBlocks`, this state is explicitly persisted to localStorage — see the Local Practice Persistence plan).
...
**The drum machine** (`start()`/`stop()`) is a standard Web Audio "lookahead scheduler" — a coarse ~25ms `setInterval` checker that schedules any of `pattern`'s active steps whose grid time falls within a 100ms lookahead window at precise `AudioContext.currentTime` values, so timing stays sample-accurate regardless of JS timer jitter (this matters once swing timing needs to be audible, not just "close enough"). Four synthesized voices — kick, snare, closed hat, open hat (`$lib/audio/drum-voices.ts`, oscillators + a shared noise buffer, no sample files) — replaced the original single quarter-note click (`$lib/audio/metronome.ts`, now deleted) by explicit product direction. `$lib/audio/groove.ts` owns the pattern data model (16 boolean steps per voice, one bar, plus a global swing 0–100 that delays the "and" of each beat toward a triplet feel) and `$lib/audio/groove-presets.ts` ships 5 curated genre patterns (Click, Straight/Rock, Blues Shuffle, Jazz Swing, Funk). It carries no target, no evaluation, no per-beat state — same as before. Starting it happens inside the "Start Metronome" button's click handler, the same user-gesture requirement Live Input's `getUserMedia` call already lives under.
...
**Every fret's label** shows its interval relative to `scalePractice.root` stacked above the note name (e.g. "R / A", "b3 / C") — computed locally in `FretCell.svelte` via `intervalFromRoot`/`noteNameForPosition` against `scalePractice.root`, not `fretfield.root` (Scale Practice has exactly one root, unlike Scale Blocks' several, so this is well-defined here in a way it isn't there). The root is labeled **"R"**, the mode's one deliberate deviation from the numeric "1" every other mode uses for root — everywhere else in the app "1" is the convention (§3.1); here it isn't, by explicit request. Shown for _every_ fret, not just ones in the configured scale — a chromatic passing tone still reads its own honest interval; scale membership itself is signaled twice, not once: the background tint described above _and_ the label rendering bold, so the distinction survives even if color is hard to perceive (AGENTS.md §7's non-color-signal rule). Always both interval and note name together, deliberately not gated behind the shared Intervals/Notes/Both toggle other modes expose.

**The UI** (`ScalePracticeControls.svelte` + `DrumMachineControls.svelte`) visually separates the root/scale/zone fields (which drive the highlight, and take effect immediately) from a distinct drum-machine sub-panel below a divider (Genre preset, Tempo, Swing, a 4-voice × 16-step toggle grid, Start/Stop, and a "My Grooves" save/rename/delete list via the shared Saved Material Library factory) — the separation itself is the point: nothing about Start/Stop should read as gating the highlight.

**Not built (yet):** sample-based drum sounds (every voice stays synthesized, no asset-loading pipeline), per-step velocity/accent editing (genre character comes from step placement and swing, not a velocity grid), tempo ramps (BPM is adjusted by hand), a mute toggle, persistent session stats, any notion of "correct"/"on time" grading (deliberately removed — see below), and Live Input/Guided Practice driving this mode the way they drive the four single-chord modes.

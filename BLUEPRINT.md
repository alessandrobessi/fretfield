...
**The Acid Bass Engine** (`$lib/acid-bass/` — see AGENTS.md §4 for the full module breakdown, driving `$lib/audio/acid-bass-voice.ts`) is a third accompaniment layer built entirely inside the Groove Engine above: a monophonic, 303-inspired synth bass voice, off by default, sharing the drum engine's persistence, migration, and `A`/`B`/`F`/`T` arrangement roles rather than an independent bass arrangement.

- **Data model** (`acid-bass/types.ts`): `AcidBassStep` (`active`, `interval: IntervalId`, `octave`, `accent`, `slide`) reuses the app's existing `IntervalId` (`$lib/music/intervals.ts`) rather than a bespoke numeric interval type — one canonical interval representation everywhere. `AcidBassPatch` holds six `0`–`100` macro controls (Wave — Saw/Square, Tone, Resonance, Motion, Decay, Drive); `AcidBassState` (`enabled`, `patch`, `patterns: Record<PatternRole, AcidBassPattern>`) nests inside `Groove.acidBass`, resized in lockstep with the drum patterns whenever the meter changes.
- **Voice** (`$lib/audio/acid-bass-voice.ts`'s `createAcidBassVoice`): unlike the drum kit's and chord pad's fire-and-forget per-hit trigger functions, this builds one persistent node graph at creation — two always-running oscillators crossfaded by waveform, one shared lowpass filter/VCA/drive chain — that lives across the whole playback session, because slide/legato behavior needs to glide one running oscillator's frequency rather than crossfade between discrete note events. A step's `slide` glides toward the _immediately following_ step's own pitch only (never cross-bar); the "no re-trigger at the slide destination" requirement is handled by a `legato` trigger flag on `schedule()` — the source step's own call ramps frequency toward the destination near the end of its duration without releasing, and the destination step's own later call passes `legato: true` to skip the attack/filter-envelope retrigger while still scheduling its own release.
- **Transport integration**: schedules off the exact same `GrooveTransport`/swung `stepOffsetMs` timing the drums already use — never a second scheduler or a second `AudioContext`. `scale-practice.svelte.ts` resolves the harmonic root per bar (the active progression chord's root, or the practice `root` with no progression) and each step's interval → MIDI via `acid-bass/resolve.ts`'s `resolveAcidStepMidi`, reusing `$lib/music/intervals`'s `intervalSemitones` and `$lib/audio/note-mapping.ts`'s existing `midiToFrequency` — never a second harmony or pitch-mapping engine.
- **UI**: Bass on/off in `PracticeSessionBar.svelte` (PLAY tier); patch macros on `BandPanel.svelte`'s Bass tab (ADJUST tier), applied live to the running voice via `setPatch()` without restarting playback; step grid + selected-step editor inside `GrooveEditor.svelte`'s "Bass Steps" sub-tab (EDIT tier), toggled against "Drum Steps" — both read the same `selectedPatternRole`, since there's no independent bass pattern-role picker.
- **Not built (and deliberately out of scope):** DAW features (piano-roll, automation lanes, MIDI export), a hardware-clone 303 faceplate UI, AI-generated bass lines, cross-bar slide, an Intensity/velocity-gating concept of its own (every active step always sounds — no `minIntensity` equivalent), and sample-based tone generation (stays two oscillators + a filter).

...
- **Chord Pad FX** (`$lib/chord-pad-fx/`/`$lib/audio/chord-pad-fx.ts` — user-requested, 2026-08, not part of any spec doc; see AGENTS.md §4): the chord-backing pad (`$lib/audio/chord-voices.ts`'s `triggerChordPad`, still a fire-and-forget per-hit trigger function) now routes every hit through a persistent six-effect rack instead of straight to `ctx.destination`, built in two stages: Reverb/Delay/Chorus first, then Phaser/Flanger/Tremolo (`ChordPadFxState.version` bumped `1 → 2`, migrated). Signal order: Chorus → Phaser → Flanger → Tremolo → Delay → Reverb → destination — the four modulation effects grouped ahead of the two time-based ones. Each effect is off by default and controlled via its own `HardwarePanel` (Harmony tab), matching Acid Bass's own panel language; Tremolo alone has no Mix knob, since it's a genuine in-series amplitude modulation rather than a dry/wet insert like the other five. Reverb is a small algorithmic (Freeverb/Schroeder-style) comb+allpass network built from plain nodes, not a convolution reverb — this app has no external impulse-response asset anywhere; Phaser reuses that same native-allpass idea as an LFO-swept 4-stage series; Flanger is Chorus's own modulated-delay primitive plus a feedback loop. `ChordPadFxState` nests on `Groove` as `chordPadFx`, sibling to `acidBass`, independently versioned/migrated.

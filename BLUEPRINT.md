...
- **Migration** (`groove/migrate.ts`): reads any prior persisted shape (the original single boolean pattern, the pre-Feel-engine `Groove` with a bare `swing` number, or the pre-time-signature `Groove` missing `timeSignature`) and coerces it into the current model, so existing saved grooves keep working across the data model's evolution.

...
- **Tuner** (user-requested, 2026-08, direct follow-up to Radio Mode's removal below in the historical record — see AGENTS.md): a "Tuner" tab on `BandPanel.svelte` (between Mixer and Editor), string-aware rather than a generic chromatic tuner — it identifies which of the bass's 4 open strings (E/A/D/G, `STANDARD_4_STRING_ABSOLUTE_TUNING`) a `liveInput.detectedNote` is closest to, shows cents off that string, and flags a note more than `MAX_SEMITONES_FROM_OPEN_STRING` (3) semitones from any open string as a fretted note rather than a mistuned one. All math lives in a new pure module, `src/lib/music/tuner.ts` (`findClosestOpenString`/`centsFromOpenString`/`classifyTunerStatus`) — no new pitch detection, since `DetectedNote.midi`/`.cents` already carry exactly what a tuner needs. `TunerControls.svelte` reads the `liveInput` singleton directly, the same pattern `LiveMusicalContext.svelte` already uses, with no enable flow of its own — Live Input's existing header toggle is the only way to start it. In-tune/out-of-tune reuse the existing yellow/red brand semantics rather than a new green token, since the palette (`app.css`) is deliberately monochrome-plus-signal-red.

...
- **Chord Pad FX** (`$lib/chord-pad-fx/`/`$lib/audio/chord-pad-fx.ts` — user-requested, 2026-08, not part of any spec doc; see AGENTS.md §4): the chord-backing pad (`$lib/audio/chord-voices.ts`'s `triggerChordPad`, still a fire-and-forget per-hit trigger function) now routes every hit through a persistent Reverb/Delay/Chorus rack instead of straight to `ctx.destination` — stage 1 of 2, Phaser/Flanger/Tremolo deferred. Each effect is off by default and controlled via its own `HardwarePanel` (Harmony tab), matching Acid Bass's own panel language. Reverb is a small algorithmic (Freeverb/Schroeder-style) comb+allpass network built from plain nodes, not a convolution reverb — this app has no external impulse-response asset anywhere. `ChordPadFxState` nests on `Groove` as `chordPadFx`, sibling to `acidBass`, independently versioned/migrated.

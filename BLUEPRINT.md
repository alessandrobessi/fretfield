...
**Radio Mode** (`/radio`, user-requested, 2026-08) is not a third destination and does not touch this two-field model at all — it's a standalone, unlisted page (not in the nav, `noindex`, not in `static/sitemap.xml`) for streaming FretField autonomously on YouTube, reached only by direct URL (what an operator points OBS's Browser Source at). It reuses the exact `scalePractice` singleton and its existing public setters as its whole playback engine; the only new logic is `RadioDirector` (`$lib/stores/radio-director.ts`), which just calls those setters on a timer to roll a fresh root/progression/bass-style/tempo combo every 90-180 seconds (the groove/genre itself is pinned to Trance, by explicit user request — not part of the rotation), plus a new full-bleed audio-reactive visualizer (`RadioVisualizer.svelte`) in place of the fretboard. See §21's own Radio Mode entry for the full mechanics.
...
This was built (as "Voice-Leading Paths") and later deleted entirely by explicit product request — see §0/§16. Kept below as a historical record only. Acid Bass Intelligence V4 (§21) later adds its own `voice-leading.ts`, but that is new, internal-only scoring logic feeding the bassline generator, not a UI feature and not a resurrection of this deleted mode — it must not import or restore any of the deleted `connection-score.ts`/`voice-leading-paths.ts`/`local-fields.ts` files.
...
- **Voices** (`$lib/audio/drum-voices.ts`): six synthesized voices — kick, snare, closed hat, open hat, ride, rim (oscillators + a shared noise buffer, no sample files) — each taking a `gain` parameter that step velocity maps directly onto, and (as of the Mixer, below) a `destinationNode` parameter routing every hit through the shared `DrumsBus` instead of `ctx.destination` directly.
- **Mixer** (user-requested, 2026-08): a Drums/Chords/Bass channel-volume strip on its own `BandPanel.svelte` tab (between Bass and Editor) — first built as an always-visible strip above every tab, then corrected by direct user feedback ("mixer should have its own tab, it should not be in the harmony section") to a normal tab like the other four, since it wasn't reading as a separate voice at all when shown everywhere; each fader is the same rotary `Knob` component every other volume/level control in the app uses (e.g. Acid Bass's OUTPUT > Volume), not a plain `<input type="range">` — restyled after a follow-up user request ("can the sliders be more aligned to the style of the app?"). All three channels share one `volumeToGain` headroom curve (`$lib/audio/gain.ts`, 0-100 → 0-0.9 gain), extracted from what used to be Acid Bass's own private helper. Drums route through a new persistent `DrumsBus` (`createDrumsBus`, mirroring `ChordPadFxBus`'s own "one bus, `setVolume` hides the raw AudioParam" shape); Chords gained a `channelGain` stage appended after the FX rack's existing dry/wet sum. `drumsVolume`/`chordsVolume` (0-100, default 100) persist as top-level session state alongside `intensity` — not nested in `Groove`, since a mix balance is a listening preference, not authored musical content. Bass has no sibling field of its own: its fader is the same `patch.output.volume` the OUTPUT panel's existing Volume knob already controls.
- **Chord Pad FX** (`$lib/chord-pad-fx/`/`$lib/audio/chord-pad-fx.ts` — user-requested, 2026-08, not part of any spec doc; see AGENTS.md §4): the chord-backing pad (`$lib/audio/chord-voices.ts`'s `triggerChordPad`, still a fire-and-forget per-hit trigger function) now routes every hit through a persistent seven-effect rack instead of straight to `ctx.destination`, built in three stages: Reverb/Delay/Chorus first, then Phaser/Flanger/Tremolo, then Fuzz (`ChordPadFxState.version` bumped `1 → 2 → 3`, migrated at each step). Signal order: Fuzz → Chorus → Phaser → Flanger → Tremolo → Delay → Reverb → destination — Fuzz first (distortion ahead of modulation and time-based effects, the standard pedalboard convention), then the four modulation effects grouped ahead of the two time-based ones. Each effect is off by default and controlled via its own `HardwarePanel` (Harmony tab), matching Acid Bass's own panel language; Tremolo alone has no Mix knob, since it's a genuine in-series amplitude modulation rather than a dry/wet insert like the other six. Reverb is a small algorithmic (Freeverb/Schroeder-style) comb+allpass network built from plain nodes, not a convolution reverb — this app has no external impulse-response asset anywhere; Phaser reuses that same native-allpass idea as an LFO-swept 4-stage series; Flanger is Chorus's own modulated-delay primitive plus a feedback loop; Fuzz is one fixed hard-clip `WaveShaperNode` curve with Drive as its pre-gain, the same idiom Acid Bass's own `driveToPregain`/`saturationToPregain` already established. `ChordPadFxState` nests on `Groove` as `chordPadFx`, sibling to `acidBass`, independently versioned/migrated.
- **Feel + Intensity** (`groove/feel.ts`/`intensity.ts`): Feel (Straight/Shuffle/Swing) + Amount replace a bare swing percentage — Straight is always 0 regardless of a leftover Amount value, and Amount disables entirely for a compound time signature. Intensity is a single global 0–100 control; any step's `minIntensity` gates whether it sounds at the current Intensity, so one authored groove can get progressively denser without needing a preset per density level. Both Amount and Intensity render as the same rotary `Knob` every other volume/level control uses (2026-08, "can the sliders be more aligned to the style of the app?") — Amount's disabled state (Straight feel, or a compound meter) now goes through `Knob`'s own `disabled`/`title` props rather than a native `<input disabled title>`.
...
- **Migration** (`groove/migrate.ts`): reads any prior persisted shape (the original single boolean pattern, the pre-Feel-engine `Groove` with a bare `swing` number, the pre-time-signature `Groove` missing `timeSignature`, the pre-Acid-Bass `Groove` missing `acidBass`, or the pre-chord-pad-FX `Groove` missing `chordPadFx`) and coerces it into the current model, so existing saved grooves keep working across the data model's evolution. A migrated old groove always gets `acidBass.enabled: false`, regardless of what its default pattern contains — an old groove must not suddenly gain audible bass on load.
...
- **UI**: Bass on/off in `PracticeSessionBar.svelte` (PLAY tier); twelve nested VCO/SUB/OSC 2/VCF/ENV/LFO 1/LFO 2/ENV MOD/ACCENT MOD/RANDOM MOD/DELAY/OUTPUT panels (the last four added by V4 M15–M17), laid out as a compact responsive grid rather than stacked full-width, a toggleable "GLOSSARY" reference panel next to the Patch picker, plus a 14-preset factory-patch picker (8 original V2 sound-design presets + 6 genre-flavored ones added at V4's M22) on `BandPanel.svelte`'s Bass tab (ADJUST tier) — every control always visible (no disclosure), applied live to the running voice via `setPatch()` without restarting playback; step grid + selected-step editor (now with Probability/Ratchet/Gate and a parameter-lock disclosure) plus pattern-wide transform buttons (Rotate/Simplify/Densify/Octave shift/Clear All Locks) inside `GrooveEditor.svelte`'s "Bass Steps" sub-tab (EDIT tier), toggled against "Drum Steps" — both read the same `selectedPatternRole`, since there's no independent bass pattern-role picker.
- **Not built (and deliberately out of scope):** DAW features (piano-roll, automation lanes, MIDI export), a hardware-clone 303 faceplate UI, LLM/API/model-generated bass lines (V4's own local/deterministic/algorithmic generator is a distinct, explicitly authorized thing — see the note below), an independent bass tempo/swing/meter, polyphony, a many-to-many modulation matrix (V4 authorizes exactly five fixed single-destination sources, not a matrix — see below), a general effects rack (V4 authorizes exactly one dedicated tempo-sync delay, not an FX rack), chord/scale-aware pattern transforms on the _manual_ patterns (the transforms above are deliberately basic — V4's separate generated-line pipeline is chord/scale-aware by design), an Intensity/velocity-gating concept of its own (every active step always sounds, modulo its own `probability` roll — no `minIntensity` equivalent), and sample-based tone generation (stays synthesized oscillators + filters).

- **Acid Bass Intelligence V4** (`~/Downloads/ACID-BASS-INTELLIGENCE-V4.md`; check `ROADMAP.md` for current status): turns Acid Bass from a synth attached to a manual step sequencer into an algorithmic bassline generator, layered strictly on top of the architecture above. **As of 2026-08, M0–M18 and M22 (final polish) are complete; M19–M21 (the `GrooveTransport` one-shot extension, Learn This Line's UI/Live Input wiring, and Shadow Mode) were explicitly skipped by user decision, not merely not-yet-reached** — M18's pure `$lib/bass-training/` domain exists and is fully tested, but nothing wires it into the UI or audio, and there is no Shadow Mode at all; confirm with the user before assuming either exists. A new pure `$lib/music/bassline/` domain (rhythm → harmonic candidates → chord/key/voice-leading selection → controlled chromaticism → register/playability realization → articulation → explanation) produces a `GeneratedBasslinePlan`, derived on demand and never persisted — only the generation settings (style, harmony mode, seed, density, chromaticism, movement, register, playability, intelligence) and the seed are saved, alongside the unchanged manual `A`/`B`/`F`/`T` patterns. A new `AcidBassMode` (`'manual' | 'generated'`) selects between them; manual stays the default/migration mode, unchanged from V3. Generation uses the existing progression/per-chord-scale/zone/tuning context already in Scale Practice — never invents a hidden chord, and stays disabled with an explanatory message when no root/progression is chosen. Sound expression grew in parallel, bounded exactly as this doctrine specifies: five fixed single-destination modulation sources (LFO 1, LFO 2, ENV, Accent, Random — never a matrix, each with its own On/Off LED and, for the three non-LFO sources, a `AcidBassAuxModScope` preview showing their actual per-_trigger_ contour shape rather than a continuous wave, since they are not LFOs), three distortion characters (Soft/Diode/Hard, Soft byte-compatible with the existing V3 curve), and one dedicated tempo-synced delay (never a general FX rack). A toggleable "GLOSSARY" reference panel (`glossary.ts`) briefly explains every control in one findable place. Two opt-in training additions — "Learn This Line" (pitch-only evaluation against a generated bar, via the existing Live Input note-onset stream) and "Shadow Mode" (the fretboard previews upcoming generated notes during ordinary playback) — are the only exception carved into Scale Practice's otherwise absolute "no grading" doctrine, and only because they're a dedicated opt-in Acid Bass session, not ordinary Scale Practice; **as of 2026-08, neither has a UI yet** (see the status note above). Still one `GrooveTransport`, one output `AudioContext`, and the same `A`/`B`/`F`/`T` arrangement — generated mode is not a second bass arrangement or a second scheduler.

**Radio Mode** (`src/routes/radio/+page.svelte`, user-requested, 2026-08 — "develop a feature that allows fretfield to play electronic music autonomously, so that I can stream it live all day"): a standalone, unlisted page reusing everything above as-is. `scale-practice.svelte.ts` gained three small, additive pieces purely for this: a `persistEnabled` flag (default `true`, checked in `persist()`) so Radio's own constant rotation never overwrites a real user's saved Practice session in the same browser's `localStorage`; a `masterAnalyser`, fanning `drumsBus.output`/`chordPadFxBus.output`/`acidBassVoice.outputAnalyser` into one tap (`ChordPadFxBus` gained a matching public `output` field for this, mirroring `DrumsBus`'s own) alongside their existing connections to `ctx.destination`; and `getMasterAnalyser()` to read it. `RadioDirector` (`$lib/stores/radio-director.ts`) is the only new playback-adjacent logic — a small, store-independent module (seven injected setter functions, not a dependency on `ScalePracticeStore` itself) that rolls a fresh root/progression/Acid-Bass-generation-style/tempo combo every 90-180 seconds, never repeating any one field back-to-back. The groove/genre itself is pinned to Trance (user-requested, 2026-08 — "music should always be trance"), not part of the rotation; `GROOVE_PRESET_IDS` originally cycled the full preset pool with a genre-appropriate tempo range per preset (a flat global BPM range would've put bossa nova and drum-and-bass in the same pool) before being narrowed to this one fixed choice — the tempo-range lookup table stayed, now always resolving to Trance's own 130-145 range. Every rotation calls `setAcidBassEnabled(true)`/`setAcidBassMode('generated')` in addition to `setGroove` — a real bug, not defensive padding: a `GroovePreset`'s own baked-in `Groove` carries Acid Bass's default disabled/manual state (`createEmptyGroove`'s own default), so `setGroove` alone silently turned the bass back off on every single rotation, which is exactly what a user reported ("i can't hear the bass playing") before this fix. `RadioVisualizer.svelte` replaces the fretboard with a full-bleed, audio-reactive radial spectrum ring in the app's own yellow/black brand palette, reading `getMasterAnalyser()` every frame the same "genuine live tap" way `AcidBassAudioScope.svelte` already does. The page itself needs one user click to start (Web Audio's own gesture requirement, the same one every Play button in this app already goes through) and then runs unattended; it is excluded from search (`noindex, nofollow`, not in `static/sitemap.xml`) and not linked from anywhere in the app — see §0's own Radio Mode note for why this isn't a third destination.
...
- notation editor
- tabs editor
- LLM/API/model-generated bass lines (local deterministic algorithmic bassline generation is separately authorized by Acid Bass Intelligence V4 — see §21's note)
- community content
- guitar support
